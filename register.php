...
if (isset($_POST['submit'])) {
	...
	if ($password == $cpassword) {
		...
		if (!$result->num_rows > 0) {
			$sql = "INSERT INTO users (username, name, email, phone, useraddress, password)
					VALUES ('$username', '$name', '$email', '$phone', '$useraddress', '$password')";
			$result = mysqli_query($conn, $sql);
			if ($result) {
...
<div class="register_body">
	<div class="container register_container">
		<form action="" method="POST" class="login-email">
			...
			<div class="input-group">
				<input type="text" placeholder="Address" name="useraddress" value="<?php echo $useraddress; ?>" required>
			</div>
			<div class="input-group">
